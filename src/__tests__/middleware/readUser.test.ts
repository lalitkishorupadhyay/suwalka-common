...
describe('readUser middleware', () => {
  ...
  it('calls next(UnauthenticatedError) when X-Userinfo is an array (non-string)', () => {
    const req = mockReq({ headers: { 'x-userinfo': ['val1', 'val2'] } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthenticatedError));
    expect(next.mock.calls[0][0].message).toBe('Missing X-Userinfo header');
  ...
  it('calls next(UnauthenticatedError) when Base64 decodes to invalid JSON', () => {
    ...
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthenticatedError));
  ...
  it('calls next(UnauthenticatedError) when sub is missing (falsy)', () => {
    const raw = encodeUserinfo({ email: 'x@y.com' });
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthenticatedError));
  ...
  it('calls next(UnauthenticatedError) when sub is a number (truthy but not string)', () => {
    const raw = encodeUserinfo({ sub: 12345, email: 'x@y.com' });
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith(expect.any(UnauthenticatedError));
  ...
  it('sets req.user and calls next() on a fully valid request', () => {
    const raw = encodeUserinfo({ sub: 'user-001', email: 'dev@suwalka.in', roles: ['DSE'] });
    const req = mockReq({
      headers: {
        'x-userinfo': raw,
  ...
  it('defaults email to empty string when email is not in the blob', () => {
    const raw = encodeUserinfo({ sub: 'user-002' });
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith();
  ...
  it('defaults roles to [] when roles are absent from the blob', () => {
    const raw = encodeUserinfo({ sub: 'user-003' });
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith();
  ...
  it('picks first element when X-Tenant-Id is an array', () => {
    const raw = encodeUserinfo({ sub: 'user-004' });
    const req = mockReq({
      headers: {
        'x-userinfo': raw,
  ...
  it('returns empty string for X-Branch-Id when header is absent', () => {
    const raw = encodeUserinfo({ sub: 'user-005' });
    const req = mockReq({ headers: { 'x-userinfo': raw } });
    const next = mockNext();
    readUser(req, mockRes(), next);
    expect(next).toHaveBeenCalledWith();
