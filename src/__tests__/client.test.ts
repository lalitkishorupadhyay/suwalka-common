...
class TestClient extends ServiceClient {
  public headers(extra?: Record<string, string>) { return this.buildHeaders(extra); }
  public callGet<T>(path: string, extra?: Record<string, string>) { return this.get<T>(path, extra); }
  public callPost<T>(path: string, body: unknown, extra?: Record<string, string>) { return this.post<T>(path, body, extra); }
  public callPatch<T>(path: string, body: unknown, extra?: Record<string, string>) { return this.patch<T>(path, body, extra); }
  public callRequest<T>(method: string, path: string, body?: unknown, extra?: Record<string, string>) {
    return this.request<T>(method, path, body, extra);
  }
}
...
function makeErrorResponse(status: number, jsonFn?: () => Promise<unknown>): Response {
  return {
    ok: false,
    status,
...
describe('ServiceClient', () => {
  ...
  describe('request — success', () => {
    it('GET without body returns parsed JSON', async () => {
      ...
      const result = await client.callGet<{ id: string }>('/employees/1');
    ...
    it('POST with body serialises it as JSON', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        `${BASE}/employees`,
        expect.objectContaining({ method: 'POST', body: JSON.stringify({ name: 'Ravi' }) })
      );
    });

    it('PATCH works', async () => {
      fetchSpy.mockResolvedValueOnce(makeOkResponse({ id: '3' }));
      const result = await client.callPatch<{ id: string }>('/employees/3', { status: 'INACTIVE' });
  ...
  describe('request — non-ok HTTP response', () => {
    ...
    it('sets errorBody to null when response.json() throws on a non-ok response', async () => {
      fetchSpy.mockResolvedValueOnce(
        makeErrorResponse(500, () => Promise.reject(new Error('not json')))
      );
      const err = await client.callGet('/path').catch(e => e);
      expect(err).toBeInstanceOf(DependencyError);
